...
# version for our macOS 12 baseline and matching experimental SHA256 ABI.
set -euo pipefail
cd "$(dirname "$0")/.."
...
cmake -S "$WORK/source" -B "$WORK/compiled" \
  ...
  -DCMAKE_OSX_DEPLOYMENT_TARGET=12.0 -DBUILD_SHARED_LIBS=ON \
...
libraries = [p for p in (work / 'compiled').rglob('libgit2*.dylib') if not p.is_symlink()]
...
provenance = {
    'libgit2': '1.9.7', 'minimumMacOS': '12.0', 'architecture': 'arm64',
...
print('Prepared arm64 libgit2 1.9.7 for macOS 12 using system TLS; no Homebrew runtime dependencies.')
PY
